...
exports.signup = catchAsync(async (req, res, next) => {
  // creating new document in the database
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //   1) Check if email and password exist in the database
  if (!email || !password) {
...
exports.isLoggedIn = catchAsync(async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    // 1) verify token
    console.log('in the token verify');
    const decoded = await promisify(jwt.verify)(
      req.cookies.jwt,
      `${process.env.JWT_Secret}`
    );
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) If token has not TokenExpiredError, and there is user, set the new password
  if (!user) {
